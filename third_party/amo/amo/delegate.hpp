...
#ifndef AMO_DELEGATE_HPP
#define AMO_DELEGATE_HPP
...
// R is the type the compiler uses in compiling the template. For VC6,
// it cannot be void. DesiredR is the real type which is returned from
...
#define FUNCTION_GEN(val)\
	template<TEMPLATE_PARAM_TYPE##val>\
class delegate##val {\
private:\
	typedef typename detail::DefaultVoidToVoid<R>::type DesiredR;\
	typedef DesiredR (*StaticFunctionPtr)(FUNC_PARAM##val);\
	typedef R (*UnvoidStaticFunctionPtr)(FUNC_PARAM##val);\
	typedef R (detail::GenericClass::*GenericMemFn)(FUNC_PARAM##val);\
	typedef detail::ClosurePtr<GenericMemFn, StaticFunctionPtr, UnvoidStaticFunctionPtr> ClosureType;\
	ClosureType m_Closure;\
public:\
	typedef delegate##val type;\
	delegate##val() { clear(); }\
	delegate##val(const delegate##val &x) {\
		m_Closure.CopyFrom(this, x.m_Closure); }\
	void operator = (const delegate##val &x)  {\
		m_Closure.CopyFrom(this, x.m_Closure); }\
	bool operator ==(const delegate##val &x) const {\
	...
	delegate##val(DesiredR (X::* function_to_bind)(FUNC_PARAM##val), Y *pthis ) {\
		m_Closure.bindmemfunc(detail::implicit_cast<X*>(pthis), function_to_bind); }\
	template < class X, class Y >\
	inline void bind( DesiredR (X::* function_to_bind)(FUNC_PARAM##val), Y *pthis) {\
		m_Closure.bindmemfunc(detail::implicit_cast<X*>(pthis), function_to_bind);	}\
	template < class X, class Y >\
	delegate##val(DesiredR (X::* function_to_bind)(FUNC_PARAM##val) const, const Y *pthis) {\
		m_Closure.bindconstmemfunc(detail::implicit_cast<const X*>(pthis), function_to_bind);	}\
	template < class X, class Y >\
	inline void bind( DesiredR (X::* function_to_bind)(FUNC_PARAM##val) const, const Y *pthis) {\
		m_Closure.bindconstmemfunc(detail::implicit_cast<const X *>(pthis), function_to_bind);	}\
	delegate##val(DesiredR (*function_to_bind)(FUNC_PARAM##val) ) {\
		bind(function_to_bind);	}\
	void operator = (DesiredR (*function_to_bind)(FUNC_PARAM##val) ) {\
		bind(function_to_bind);	}\
	inline void bind(DesiredR (*function_to_bind)(FUNC_PARAM##val)) {\
		m_Closure.bindstaticfunc(this, &delegate##val::InvokeStaticFunction, \
		function_to_bind); }\
	R operator() (FUNC_PARAM##val) const {\
	return (m_Closure.GetClosureThis()->*(m_Closure.GetClosureMemPtr()))(FUNC_PARAM_VAL##val); }\
private:\
...
private:	\
	R InvokeStaticFunction(FUNC_PARAM##val) const {\
		return (*(m_Closure.GetStaticFunction()))(FUNC_PARAM_VAL##val); }\
};
...
#define FFFF_GEN(val)\
	template<TEMPLATE_PARAM_TYPE##val>\
class delegate< R ( FUNC_PARAM_TYPE##val ) >\
	: public delegate##val < TEMPLATE_PARAM_TYPE##val >\
{\
public:\
	typedef delegate##val < TEMPLATE_PARAM_TYPE##val > BaseType;\
	\
	...
	\
	template < class X, class Y >\
	delegate(R (X::* function_to_bind)( FUNC_PARAM##val ), Y * pthis)\
		: BaseType( function_to_bind, pthis)  { }\
		\
	template < class X, class Y >\
	delegate(R (X::* function_to_bind)( FUNC_PARAM##val ) const, const Y *pthis)\
		: BaseType( function_to_bind, pthis)\
	{  }\
	\
	delegate(R (*function_to_bind)( FUNC_PARAM##val ))\
...
#define BIND_GEN(val)\
	template <TEMPLATE_PARAM_TYPE##val>\
	delegate< R ( FUNC_PARAM_TYPE##val ) > make_delegate(R (*func)(FUNC_PARAM##val ), ...)\
{ \
	return delegate< R (FUNC_PARAM_TYPE##val ) >(func);\
}\
	\
	template <TEMPLATE_PARAM_TYPE##val, class X, class Y>\
	delegate< R ( FUNC_PARAM_TYPE##val ) > make_delegate(R (X::*func)(FUNC_PARAM##val ), Y * y, ...)\
{ \
	return delegate< R (FUNC_PARAM_TYPE##val ) >(func, y);\
}\
	\
	template <TEMPLATE_PARAM_TYPE##val, class X, class Y>\
	delegate< R ( FUNC_PARAM_TYPE##val ) > make_delegate(R (X::*func)(FUNC_PARAM##val ) const, Y * y, ...)\
{ \
	return delegate< R (FUNC_PARAM_TYPE##val ) >(func, y);\
}
...
#endif // !defined(AMO_FUNCTION_HPP)
