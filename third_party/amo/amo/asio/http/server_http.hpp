...
namespace SimpleWeb {
    ...
    public:
        class Response : public std::enable_shared_from_this<Response>,
            ...
            Response(std::shared_ptr<Session> session,
                     long timeout_content) noexcept : std::ostream(&streambuf),
                session(std::move(session)), timeout_content(timeout_content) {}
        ...
        public:
            ...
            noexcept {
                ...
                auto self =
                    this->shared_from_this(); // Keep Response instance alive through the following async_write
                asio::async_write(*session->connection->socket, streambuf, [self,
                callback](const error_code & ec, std::size_t /*bytes_transferred*/) {
            ...
            void write(StatusCode status_code, const std::string &content,
                       const CaseInsensitiveMultimap &header = CaseInsensitiveMultimap()) {
                *this << "HTTP/1.1 " << SimpleWeb::status_code(status_code) << "\r\n";
        ...
        class Request {
            ...
            Request(std::size_t max_request_streambuf_size,
                ...
                : streambuf(max_request_streambuf_size), content(streambuf),
    ...
    protected:
        ...
        public:
            Session(std::size_t max_request_streambuf_size,
                    std::shared_ptr<Connection> connection) noexcept : connection(std::move(
                                    connection)) {
    ...
    private:
        ...
        public:
            regex_orderable(const char *regex_cstr) : regex::regex(regex_cstr),
                str(regex_cstr) {}
            regex_orderable(std::string regex_str) : regex::regex(regex_str),
    ...
    public:
        ...
        std::map<regex_orderable, std::map<std::string, std::function<void(std::shared_ptr<typename ServerBase<socket_type>::Response>, std::shared_ptr<typename ServerBase<socket_type>::Request>)>>>
        resource;
        
        std::map<std::string, std::function<void(std::shared_ptr<typename ServerBase<socket_type>::Response>, std::shared_ptr<typename ServerBase<socket_type>::Request>)>>
                default_resource;
                
        std::function<void(std::shared_ptr<typename ServerBase<socket_type>::Request>, const error_code &)>
        on_error;
        ...
        unsigned short bind() {
            ...
            if (!acceptor) {
                acceptor = std::unique_ptr<asio::ip::tcp::acceptor>(new asio::ip::tcp::acceptor(
                               *io_service));
            }
    ...
    protected:
        ...
        ServerBase(unsigned short port) noexcept : config(port),
        ...
        std::shared_ptr<Connection> create_connection(Args &&... args) noexcept {
            ...
            auto connection = std::shared_ptr<Connection>(new Connection(handler_runner,
                              std::forward<Args>(args)...), [connections,
            connections_mutex](Connection * connection) {
                {
                    std::unique_lock<std::mutex> lock(*connections_mutex);
        ...
        void read(const std::shared_ptr<Session> &session) {
            ...
            std::size_t bytes_transferred) {
                ...
                if ((!ec || ec == asio::error::not_found) &&
                    ...
                    auto response = std::shared_ptr<Response>(new Response(session,
                ...
                if (!ec) {
                    ...
                    std::size_t num_additional_bytes = session->request->streambuf.size() -
                                                       bytes_transferred;
                                                       
                    if (!RequestMessage::parse(session->request->content, session->request->method,
                                               session->request->path,
                                               session->request->query_string, session->request->http_version,
                    ...
                    if (header_it != session->request->header.end()) {
                        ...
                        if (content_length > num_additional_bytes) {
                            ...
                            asio::async_read(*session->connection->socket, session->request->streambuf,
                                             asio::transfer_exactly(content_length - num_additional_bytes), [this,
        ...
        void read_chunked_transfer_encoded(const std::shared_ptr<Session> &session,
            ...
            asio::async_read_until(*session->connection->socket,
                ...
                if (!ec) {
                    ...
                    auto num_additional_bytes = session->request->streambuf.size() -
                                                bytes_transferred;
        ...
        void read_chunked_transfer_encoded_chunk(const std::shared_ptr<Session>
            ...
            if (length > 0) {
                ...
                session->request->content.read(buffer.get(),
        ...
        void write(const std::shared_ptr<Session> &session,
                   std::function<void(std::shared_ptr<typename ServerBase<socket_type>::Response>, std::shared_ptr<typename ServerBase<socket_type>::Request>)>
                   &resource_function) {
            session->connection->set_timeout(config.timeout_content);
            auto response = std::shared_ptr<Response>(new Response(session,
            config.timeout_content), [this](Response * response_ptr) {
    ...
    protected:
        void accept() override {
            ...
            acceptor->async_accept(*connection->socket, [this,
            connection](const error_code & ec) {
                ...
                auto session = std::make_shared<Session>(config.max_request_streambuf_size,
                               connection);
