...
namespace DuiLib {
    ...
    std::map<HWND, ShadowWindow*>* ShadowWindow::s_Shadowmap = new std::map < HWND, ShadowWindow* > ;
    bool ShadowWindow::s_bHasInit = FALSE;
    
    ShadowWindow::ShadowWindow(void)
        : m_hWnd((HWND)NULL)
        ...
        m_rcHoleOffset = { 5, 5, 5, 5, };
    }
    
    ShadowWindow::~ShadowWindow(void) {
    }
    
    RECT ShadowWindow::getHoleOffset() const {
        return m_rcHoleOffset;
    }
    
    void ShadowWindow::setHoleOffset(RECT val) {
        m_rcHoleOffset = val;
    }
    
    bool ShadowWindow::Initialize(HINSTANCE hInstance) {
        if (s_bHasInit) {
            return false;
        }
    ...
    void ShadowWindow::Create(CPaintManagerUI* pPaintManager) {
        if (!m_bIsShowShadow) {
            return;
        }
...
#pragma warning(default: 4311)
    ...
    LRESULT CALLBACK ShadowWindow::ParentProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
        _ASSERT(s_Shadowmap->find(hwnd) != s_Shadowmap->end());	// Shadow must have been attached
        
        ShadowWindow* pThis = (*s_Shadowmap)[hwnd];
...
#pragma warning(default: 4312)
    ...
    void ShadowWindow::Update(HWND hParent) {
        ...
        if (m_bIsImageMode) {
            ...
            RECT rcCorner = { m_rcShadowCorner.left + m_rcHoleOffset.left, m_rcShadowCorner.top + m_rcHoleOffset.top,
                              m_rcShadowCorner.right + m_rcHoleOffset.right, m_rcShadowCorner.bottom + m_rcHoleOffset.bottom
                            };
            CRenderEngine::DrawImage(hMemDC, data->hBitmap, rcPaint, rcPaint, rcBmpPart, rcCorner, true, 255, true);
    ...
    void ShadowWindow::ShowShadow(bool bShow) {
        m_bIsShowShadow = bShow;
    }
    
    bool ShadowWindow::IsShowShadow() const {
        return m_bIsShowShadow;
    }
    
    bool ShadowWindow::SetSize(int NewSize) {
    ...
    bool ShadowWindow::SetSharpness(unsigned int NewSharpness) {
        if (NewSharpness > 20) {
            return false;
        }
    ...
    bool ShadowWindow::SetDarkness(unsigned int NewDarkness) {
        if (NewDarkness > 255) {
            return false;
        }
    ...
    bool ShadowWindow::SetPosition(int NewXOffset, int NewYOffset) {
    ...
    bool ShadowWindow::SetColor(COLORREF NewColor) {
        m_Color = NewColor;
    ...
    bool ShadowWindow::SetImage(LPCTSTR szImage) {
        if (szImage == NULL) {
            return false;
        }
    ...
    bool ShadowWindow::SetShadowCorner(RECT rcCorner) {
    ...
    void ShadowWindow::Show(bool bShow /*= true*/) {
        m_bShowWindow = bShow;
        
        if (bShow && (m_Status & SS_VISABLE)) {
