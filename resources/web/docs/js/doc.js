...
//                      $pageBody.children('.loader').addClass('with-error');
    ...
    var query = function(keyString) {
    	...
    	setTimeout(function() {
           	...
           	$('ul.topics').each(function(i, topic){
           		var allHidden = true;
           		$(topic).find('li').each(function(index, element){
           			var desc = $(element).find('a').text().toLowerCase();
	        		var queryString = lastQueryString.toLowerCase();
	        		
		        	if(desc.indexOf(queryString) != -1){
		        		$(element).removeClass('hidden2'); 
		        		allHidden = false;
		        	}else{
		        		$(element).addClass('hidden2'); 
		        	}
           		});
    ...
    var openPage = function($section, section, topic) {
    	var index = -1;
    	if(typeof topic == 'undefined' && lastQueryString && lastQueryString != ''){
    	 	
    	 	// 先正常查找
    		for(var i = 0; i < section.topics.length; ++i){ 
    			if(lastQueryString == section.topics[i].name){
    				index = i;
    				break;
    			}
    		}
    		...
    		if(index == -1){
	    		...
	    		for(var i = 0; i < section.topics.length; ++i){
	    			 var name = section.topics[i].name.toLowerCase();
	    			if(queryString == name){
	    				index = i;
	    				break;
	    			}
    ...
    var initChangeView = function() {
        var changePageView = function(pageViewLayout, notSave) {
            ...
            var isDoubleView = pageViewLayout === 'double' && $(window).width() >= 960;
